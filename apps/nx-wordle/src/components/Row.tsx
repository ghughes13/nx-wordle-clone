...
const Row = ({
  ...
  useEffect(() => {
    ...
    const validateWord = () => {
      const tempTileStatesArray : string[] = [];
      const correctWordAsArray = correctWord && correctWord.split("");

      if(correctWord === rowWordGuess) {
        setGameOver(true);
      }
    ...
    if(rowWordGuess.length && !tilesStates.length) {
      validateWord();
    }
